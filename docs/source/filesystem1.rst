...
Linux'ta sistem genelinde tek bir sayfa önbelleği yoktur. Her inode nesnesinin ayrı bir sayfa önbelleği vardır. Ancak biz 
...
tutmaktadır. Buradaki ``path`` yapısı da şöyle tanımlanmıştır:
...
``include/linux/fdtable.h`` dosyası içerisinde şöyle tanımlanmıştır:
...
Buradaki ``fdtable`` yapısı da şöyle tanımlanmıştır:
...
Burada görmüş olduğunuz ``fd_set`` yapısı "bit dizilerini" temsil etmektedir. Bu yapı şöyle tanımlanmıştır:
...
Dosya nesnesini temsil eden ``file`` yapısı güncel çekirdeklerde``include/linux/fs.h`` dosyasında aşağıdaki gibi tanımlanmıştır:
...
Yapı tanımlamasındaki bazı elemanların bazı konfigürasyon seçenekleri seçildiğinde yapıya dahil edildiğine dikkat
...
Çekirdeğin 2.2'li versiyonlarında yapı şöyle tanımlanmıştır:
...
Güncel çekirdeklerde ``inode`` yapısı *include/linux/fs.h* dosyası içerisinde şöyle tanımlanmıştır:
...
``m_inode`` ismiyle şöyle tanımlanmıştır:
...
``path`` yapısı da şöyle tanımlanmıştır:
...
Kursun yapıldığı makinede bulunan 6.9.2 çekirdeğinde ``struct fd`` yapısı şöyle tanımlanmıştır:
...
Burada aslında ``atomic_long_t`` türü bir yapı biçiminde tanımlanmıştır:
...
yapısı güncel çekirdeklerde şöyle tanımlanmıştır:
...
Eskiden bu yapı şöyle tanımlanmıştır:
...
``hlist_bl_node`` yapısı da şöyle tanımlanmıştır:
...
nedenle dentry önbelleği için LRU bağlı listeleri ``include/linux/fs.h`` dosyası içerisinde tanımlanmış olan 
...
Buradaki ``list_lru`` yapısı da ``include/linux/list_lru.h`` dosyası içerisinde şöyle tanımlanmıştır:
...
yine ``include/linux/list_lru.h`` dosyası içerisinde şöyle tanımlanmıştır:
...
Buradaki ``list_lru_one`` yapısı da yine *include/linux/list_lru.h* dosyasında şöyle tanımlanmıştır:
...
Aynı yapı ``fs/ext2/ext2.h`` dosyası içerisinde ``ext2_dir_entry_2`` ismiyle de tanımlanmıştır.
...
bu eleman şöyle tanımlanmıştır:
...
Güncel çekirdeklerde ``super_block`` yapısı şöyle tanımlanmıştır:
...
``file_system_type`` yapısı güncel çekirdeklerde ``include/fs.h`` dosyası içerisinde şöyle tanımlanmıştır:
...
Süper Blok Nesnesini Oluşturan Yüksek Seviyeli Fonksiyonlar: mount_nodev ve mount_bdev
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
``mount_bdev`` fonksiyonlarının prototipleri ``include/linux/fs.h`` dosyası içerisinde şöyle tanımlanmıştır:
...
Disk tabanlı bir dosya sistemini çekirdeğin yüksek seviyeli ``mount_bdev`` fonksiyonunu kullanarak
yazacak olan sistem programcıları tipik olarak ``mount`` fonksiyonunun içerisinde doğrudan bu fonksiyonu çağırırlar.
Örneğin:
...
nesnesinin içinin sistem programcısı tarafından doldurulmasına olanak sağlamaktadır.

``mount_bdev`` fonksiyonunun gerçekleştirimi ``linux/super.c`` dosyasında şöyle yapılmıştır:
...
``mount_bdev`` fonksiyonu kabaca bizim için şunları yapmaktadır:

1. ``lookup_bdev`` fonksiyonu ile ``dev_name`` parametresindeki aygıt dosyasını kullanarak blok aygıt
   sürücüsüne erişir.
2. ``sget`` fonksiyonu ile daha önce bu disk bölümü için bir süper blok nesnesi oluşturulmuş mu diye kontrol
   eder ve oluşturulmamışsa süper blok nesnesini oluşturur.
...
Sistem programcısının ``mount`` fonksiyonunu ``mount_bdev`` ya da ``mount_nodev`` fonksiyonlarının geri
dönüş değeriyle geri döndürdüğüne dikkat ediniz:
...
Kök Dizine İlişkin inode ve dentry Nesnelerinin Yaratılması
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Peki mount edilecek dosya sisteminin kök dizinine ilişkin inode ve dentry nesneleri nerede
...
- Yapının ``super_operations`` türünden ``s_ops`` elemanı süper blok işlemlerinde çağrılacak fonksiyonları
  ...
  ``super_operations`` yapısı şöyle tanımlanmıştır:
...
- Sistem programcının ``fill_super`` fonksiyonu içerisinde kök dizine ilişkin inode ve dentry nesnelerini
  ...
  ilişkin dentry nesnesinin adresini yerleştirmesi gerekir. Zaten ``mount_bdev`` fonksiyonu ``fill_super``fonksiyonu
  tarafından ``super_block`` nesnesinin ``s_root`` elemanına yerleştirilen dentry nesnesiyle geri dönmektedir. 
  Güncel çekirdeklerin içerisinde bir inode nesnesi yaratılmışsa onu gösterecek bir dentry nesnesinin yaratılmasını 
  sağlayan ``d_make_root`` isimli export  edilmiş bir çekirdek fonksiyonu da bulunmaktadır:
...
Süper Blok Listeleri
____________________

Çekirdek yaratılmış olan süper blok nesnelerini ``file_system_type`` yapısının ``fs_supers`` elemanındaki 
bağlı listede tutmaktadır. Yani her dosya sistemi türü için o dosya sistemi türünden disk bölümlerinin mount edilmesi
...
Güncel çekirdeklerde ``super_operations`` yapısı şöyle tanımlanmıştır:
...
``inode_operations`` yapısı şöyle tanımlanmıştır:
...
``dentry_operations`` yapısı şöyle tanımlanmıştır:
...
``file_operations`` yapısı ise şöyle tanımlanmıştır:
