...
**Soeu: Sistemdeki tüm proseslerin ana thread'lerine ilişkin ``task_struct`` nesneleri nasıl dolaşılır?**
...
Tüm Prosesleri Dolaşan Çekirdek Modülü
-----------------------------------------
...
sürücüyü yükledikten sonra *app* programı çalıştırılmalıdır. Bu program önce 10 tane thread yaratıp sonra
...
Makroya biz iki ``task_struct`` göstericisi veririz. Makro her yinelemede prosesin ana thread'ine ilişkin
``task_struct`` nesnesinin adresini birinci parametreyle verilen göstericinin içerisine, o prosesin
thread'lerine ilişkin ``task_struct`` nesnelerinin adresleri de ikinci parametreyle verilen göstericinin
...
yalnızca alt proseslerin ana thread'lerine ilişkin ``task_struct`` nesnelerinin bulunduğunu da
anımsayınız.
